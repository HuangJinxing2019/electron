<template>
  ...
  <a-card title="实例方法" size="small">
    <a-collapse v-model:activeKey="activeKey">
      <a-collapse-panel key="1" header="窗口标题、菜单栏、背景、任务栏、图标、进度条、刷新等相关设置">
        <FnComponent :list="styleList" />
      </a-collapse-panel>
      <a-collapse-panel key="2" header="窗口预览文件、截图、加载资源、加载文件、鼠标事件、输入光标方法">
        <FnComponent :list="otherList" />
      </a-collapse-panel>
      <a-collapse-panel key="3" header="设置/获取窗口size（大小）">
        <FnComponent :list="sizeList" />
      </a-collapse-panel>
      <a-collapse-panel key="4" header="设置/获取窗口位置">
        <FnComponent :list="positionList" />
      </a-collapse-panel>
      <a-collapse-panel key="5" header="窗口显示、隐藏、摧毁、聚焦相关方法。">
        <FnComponent :list="openClose" />
      </a-collapse-panel>
      <a-collapse-panel key="6" header="窗口状态相关">
        <FnComponent :list="statusList" />
      </a-collapse-panel>
      <a-collapse-panel key="7" header="窗口fullScreen（全屏）相关方法">
        <FnComponent :list="fullScreen" />
      </a-collapse-panel>
      <a-collapse-panel key="7" header="窗口fullScreen（全屏）相关方法">
        <FnComponent :list="fullScreen" />
      </a-collapse-panel>
      <a-collapse-panel key="8" header="windows的独有方法">
        <FnComponent :list="windowsList" />
      </a-collapse-panel>
      <a-collapse-panel key="9" header="macOS版的独有方法">
        <FnComponent :list="macOSList" />
      </a-collapse-panel>
    </a-collapse>
  </a-card>
</template>
...
import FnComponent from "./conponents/FnComponent.vue";
  ...
  const styleList = [
    ...
    { name: 'setProgressBar', desc: '设置进度条的进度值', value: 10, info: '设置0取消进度显示，具体用法请查看',linkUrl: 'https://www.electronjs.org/zh/docs/latest/api/browser-window#winsetprogressbarprogress-options', linkName: '官方文档' },
